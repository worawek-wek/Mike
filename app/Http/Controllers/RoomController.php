...
class RoomController extends Controller
{
    ...
    public function datatable(Request $request)
    {
        ...
        $latestRoomForRent = DB::table('room_for_rents as r1')
            ...
            ->whereRaw('r1.updated_at = (
                ...
                WHERE r2.occupancy_id = r1.occupancy_id
                AND r2.status = 1
            )');
        ...
        $results = Room::orderBy('rooms.name', 'ASC')
                        ...
                        ->leftJoinSub($latestRoomForRent, 'room_for_rents', function ($join) {
                            $join->on('rooms.occupancy_id', '=', 'room_for_rents.occupancy_id');
                                    // ->where('room_for_rents.status', 1);
                        })
                        ->leftJoin('contracts', 'rooms.occupancy_id', '=', 'contracts.occupancy_id')
    ...
    {
        $data['renter_contract'] = RoomForRents::where(function ($q) {
                                                    $q->whereHas('rent_bills', function ($query) {
                                                            $query->where('ref_type_id', 3)
                                                                ->where('ref_status_id', 5);
                                                        })
                                                    ->orWhereDoesntHave('rent_bills');
                                                })
...
//// เพิ่มผู้เช่า
    public function get_form_add_renter($room_id, $room_for_rent_id = null)
    {
        $data['page_url'] = 'room';
        if(@$room_for_rent_id){
            // return $room_for_rent_id;
            $room_for_rent = RoomForRents::find($room_for_rent_id);
            $data['renter_edit'] = $room_for_rent;
        }
        $room = Room::find($room_id);
...
//// ยกเลิกการจอง
    public function delete_reserve(Request $request, $occupancy_id)
    {
        
        try{
            // $occupancy = Occupancy::find($occupancy_id);
            // $occupancy->status = 3;
            // $occupancy->save();
            
            // $occupancy_other = Occupancy::where('ref_room_id', $occupancy->ref_room_id)->whereIn('status', [1,2])->where('id', '!=', $occupancy_id)->first();
            // if(!$occupancy_other){ // ถ้าไม่พบ occupancy ที่จอง หรือ มีผู้พักอาศัย
                $room = Room::where('occupancy_id', $occupancy_id)->update(['status' => 0]);
                $room = RoomForRents::where('occupancy_id', $occupancy_id)->update(['status' => 0]);
                $booking = Booking::where('occupancy_id', $occupancy_id)->update(['status_cancel' => 1]);
                
                // $room_for_rent = RoomForRents::where('occupancy_id', $occupancy_id)->get();
                // RoomForRents::where('occupancy_id', $occupancy_id)->delete();
                // foreach($room_for_rent as $room_for_rent){
                //     RentBill::where('ref_room_for_rent_id', $room_for_rent->id)->delete();
                // }
            // }
...
//// แท็บ -ย้ายออก
    ...
    public function get_move_out_detail_receipt($id)
    {
        ...
        //         // return $invoice->receipt;
                if(@$invoice->receipt[0]->payment_channel == 3){
                    $cal = $cal-($invoice->receipt[0]->total_amount ?? 0); // 2. เอามาลบยอดใบเสร็จย้ายออก ที่ชำระโดย หักจากเงินประกัน
                }
        //     }
        // }
        // if(@$receipt_1){
        ...
        if($cal >= 0){
            $html['cal'] = "<span class='text-danger'>
                                ยอดเงินประกันคืนผู้เช่า
                            &nbsp; ".number_format(abs($cal))."&nbsp; บาท</span>";
        }else{
            $html['cal'] = "<span class='text-success'>
                            เก็บเงินผู้เช่าเพิ่ม
                            &nbsp; ".number_format(abs($cal))."&nbsp; บาท</span>";
        }
        if(!$invoice){
            $html["move_out_receipt_bill"] = $this->get_move_out_form_receipt($id)->render();
            return $html;
        }
        ...
        $html["move_out_receipt_bill"] = view('room/move-out-detail-receipt', $data)->render();
        return $html;
    }
    ...
    public function move_out_detail_bad_debt_bill($id)
    {
        ...
        $html["bad_debt_bill"] = view('room/move-out-detail-bad-debt-bill', $data)->render();
        return $html;
    }
...
//// ดึงห้อง ย้ายออกหลายห้อง
    public function get_room_move_out(Request $request, $renter_id)
    {
        ...
        foreach($room as $row){
            ...
            $move_invoice_paid_1 = RentBill::where('ref_type_id', 1)->where('ref_room_id', $row->id)->where('ref_contract_id', $row->contract->id)->where('paid_on_checkout', 1)->first();
            $move_invoice_1 = RentBill::where('ref_type_id', 1)->whereNotIn('ref_status_id', [3,5])->where('ref_room_id', $row->id)->where('ref_contract_id', $row->contract->id)->first();
            
            // return $move_invoice_1->receipt[0]->payment_list;
            if($move_invoice_paid_1){
                $move_receipt_1 = Receipt::where('ref_rent_bill_id', $move_invoice_paid_1->id)->where('payment_channel', 3)->first();
                $calculate -= $move_receipt_1->total_amount ?? 0;
            }
            ...
            $row['move_invoice_paid_1'] = $move_invoice_paid_1;
            $row['move_invoice_1'] = $move_invoice_1;
    ...
    public function insert_receipt(Request $request)
    {
        ...
        try{
            ...
            if($invoice_total_amount == $receipt_total_amount){
                ...
                $r_b->ref_status_id =  2; //  5 = ชำระแล้ว
                $r_b->save();
            }
...
//             $invoice_summarize->save();
    ...
    public function move_out_submit(Request $request)
    {
        ...
        try{
            ...
            RoomForRents::where('ref_room_id', $request->room_id)->update(['status' => 0]);
            RentBill::where('ref_room_id', $request->room_id)->where('ref_status_id' , 2)->update(['ref_status_id' => 5]);
            Receipt::where('ref_room_id', $request->room_id)->update(['ref_status_id' => 5]);
            // foreach($rent_bill_wait as $rbw){
...
// ใบเสร็จเกี่ยวกับสรุปย้ายออก
            ...
            //////////////////////////////////////////////////////////////////////////////////////////////////////
                ...
                $receipt = new Receipt; // สร้าง ใบเสร็จสรุปการย้ายออก ไม่ใช่ ใบเสร็จย้ายออก
                $receipt->receipt_number =  $this->generateReceiptCode();
                $receipt->ref_room_id  =  $room->id;
                $receipt->ref_rent_bill_id  =  $request->invoice_id;
                $receipt->ref_contract_id  =  $invoice_summarize->ref_contract_id;
                $receipt->ref_renter_id  =  $request->ref_renter_id;
                $receipt->payment_format  =  $request->payment_format;
                $receipt->payment_channel  =  $request->move_out_payment_channel; // รูปแบบชำระเงิน 1=เงินสด / 2=โอนเงิน / 3/หักจากเงินประกัน
                $receipt->ref_bank_id  =  $request->ref_bank_id;
                $receipt->transfer_time  =  $request->transfer_time;
                $receipt->payment_date  =  $payment_date;
                $receipt->ref_type_id  =  7;
                $receipt->ref_status_id  =  5;
                $receipt->move_out_type  =  $request->move_out_type;
            ...
            $receipt_deposit_refun = new Receipt; // สร้าง ใบเสร็จคืนเงินประกัน
            $receipt_deposit_refun->receipt_number =  $this->generateReceiptCode();
            $receipt_deposit_refun->ref_room_id  =  $room->id;
            $receipt_deposit_refun->ref_rent_bill_id  =  $move_invoice_6->id;
            $receipt_deposit_refun->ref_contract_id  =  $invoice_summarize->ref_contract_id;
            $receipt_deposit_refun->ref_renter_id  =  $request->ref_renter_id;
            $receipt_deposit_refun->payment_format  =  $request->payment_format;
            $receipt_deposit_refun->payment_channel  =  $request->move_out_payment_channel; // รูปแบบชำระเงิน 1=เงินสด / 2=โอนเงิน / 3/หักจากเงินประกัน
            $receipt_deposit_refun->ref_bank_id  =  $request->ref_bank_id;
            $receipt_deposit_refun->transfer_time  =  $request->transfer_time;
            $receipt_deposit_refun->payment_date  =  $payment_date;
            $receipt_deposit_refun->ref_type_id  =  6;
            $receipt_deposit_refun->ref_status_id  =  5;
            $receipt_deposit_refun->move_out_type  =  $request->move_out_type;
            $receipt_deposit_refun->evidence_of_money_transfer  =  "";
            $receipt_deposit_refun->ref_user_id =  Auth::id();
            $receipt_deposit_refun->amount  =  0;
            $receipt_deposit_refun->save();

            $expenses = new IncomeExpenses;
            $expenses->type  =  1;
            $expenses->label  =  "ใบเสร็จคืนเงินประกัน";
            $expenses->amount  =  0;
            $expenses->date  =  Carbon::now();
            $expenses->time  =  date('H:i:s');
            $expenses->ref_room_id  =  $request->ref_room_id ?? $room->id;
            $expenses->ref_category_id  =  1;
            $expenses->name  =  $receipt_deposit_refun->renter->fullName();
            $expenses->address  =  $receipt_deposit_refun->renter->fullThaiAddress();
            $expenses->id_card_number  =  $receipt_deposit_refun->renter->id_card_number;
            // $expenses->branch  =  0;
            $expenses->phone  =  $receipt_deposit_refun->renter->phone;
            // $expenses->remark  =  0;
            $expenses->ref_receipt_id  =  $receipt_deposit_refun->id;
            $expenses->ref_user_id  =  Auth::id();
            $expenses->ref_branch_id  =  session("branch_id");
            $expenses->save();
            ...
            foreach($move_invoice_6->payment_list as $payment_list){
                    ...
                    $pay_list = new PaymentList; // สร้างรายการ Invoice คืนเงินประกัน ใน ใบย้ายออก
                    $pay_list->title  =  $payment_list->title;
                    $pay_list->price  =  $payment_list->price;
                    $pay_list->ref_payment_id  =  $receipt_deposit_refun->id;
                    $pay_list->document_type  =  2;
                    $pay_list->save();
                    ...
                    $pay_list = new PaymentList; // สร้างรายการ ใบเสร็จ คืนเงินประกัน ใน ใบย้ายออก
                    $pay_list->title  =  $payment_list->title;
                    $pay_list->price  =  $payment_list->price;
                    $pay_list->ref_payment_id  =  $receipt->id;
                    $pay_list->document_type  =  2;
            ...
            if($request->move_out_type == 2){
                    ...
                    $up_renter->blacklist_date  =  Carbon::now();
                    $up_renter->save();
                    // $invoice_summarize->ref_contract_id;
                    /////////////////////////////////////////////////////////////////////////////////////////////////////
                    $invoice_bad_debt = RentBill::where('ref_contract_id', $invoice_summarize->ref_contract_id)->where('ref_type_id', 5)->first(); // บิลหนี้สูญ

                    if($invoice_bad_debt){

                        $receipt_bad_debt = new Receipt; // สร้าง ใบเสร็จผู้เช่าหนี้
                        $receipt_bad_debt->receipt_number =  $this->generateReceiptCode();
                        $receipt_bad_debt->ref_room_id  =  $room->id;
                        $receipt_bad_debt->ref_rent_bill_id  =  $invoice_bad_debt->id;
                        $receipt_bad_debt->ref_contract_id  =  $invoice_summarize->ref_contract_id;
                        $receipt_bad_debt->ref_renter_id  =  $request->ref_renter_id;
                        $receipt_bad_debt->payment_format  =  $request->payment_format;
                        $receipt_bad_debt->payment_channel  =  $request->move_out_payment_channel; // รูปแบบชำระเงิน 1=เงินสด / 2=โอนเงิน / 3/หักจากเงินประกัน
                        $receipt_bad_debt->ref_bank_id  =  $request->ref_bank_id;
                        $receipt_bad_debt->transfer_time  =  $request->transfer_time;
                        $receipt_bad_debt->payment_date  =  $payment_date;
                        $receipt_bad_debt->ref_type_id  =  5; // 5 = ผู้เช่าหนี
                        $receipt_bad_debt->ref_status_id  =  5;
                        $receipt_bad_debt->move_out_type  =  $request->move_out_type;
                        $receipt_bad_debt->evidence_of_money_transfer  =  "";
                        $receipt_bad_debt->ref_user_id =  Auth::id();
                        $receipt_bad_debt->amount  =  0;
                        $receipt_bad_debt->save();

                        foreach($invoice_bad_debt->payment_list as $payment_list){
                                $pay_list = new PaymentList; // สร้างรายการ Invoice หักจากเงินประกัน ใน ใบย้ายออก
                                $pay_list->title  =  $payment_list->title;
                                $pay_list->price  =  $payment_list->price;
                                $pay_list->ref_payment_id  =  $request->invoice_id;
                                ...
                                $pay_list = new PaymentList; // สร้างรายการ ใบเสร็จ หักจากเงินประกัน ใน ใบย้ายออก
                                $pay_list->title  =  $payment_list->title;
                                $pay_list->price  =  $payment_list->price;
                                $pay_list->ref_payment_id  =  $receipt->id;
                                $pay_list->discount  =  !$payment_list->discount;
                                ...
                                $pay_list->title  =  $payment_list->title;
                                $pay_list->price  =  $payment_list->price;
                                $pay_list->ref_payment_id  =  $receipt_bad_debt->id;
                                $pay_list->document_type  =  2;
                                $pay_list->save();
                        }
                        $expenses = new IncomeExpenses;
                        $expenses->type  =  2;
                        $expenses->label  =  "ใบเสร็จผู้เช่าหนี";
                        $expenses->amount  =  abs($receipt_bad_debt->total_amount);
                        $expenses->date  =  Carbon::now();
                        $expenses->time  =  date('H:i:s');
                        $expenses->ref_room_id  =  $request->ref_room_id;
                        $expenses->ref_category_id  =  1;
                        $expenses->name  =  $receipt_bad_debt->renter->fullName();
                        $expenses->address  =  $receipt_bad_debt->renter->fullThaiAddress();
                        $expenses->id_card_number  =  $receipt_bad_debt->renter->id_card_number;
                        // $expenses->branch  =  0;
                        $expenses->phone  =  $receipt_bad_debt->renter->phone;
                        // $expenses->remark  =  0;
                        $expenses->ref_receipt_id  =  $receipt_bad_debt->id;
                        $expenses->ref_user_id  =  Auth::id();
                        $expenses->ref_branch_id  =  session("branch_id");
                        $expenses->save();
                ...
                // return $receipt_deposit_refun->id;
                // return 456;
                // $move_invoice_6 = RentBill::where('ref_type_id', 6)->where('ref_room_id', $room->id)->latest()->first(); // เงินประกัน

                $receipt_rent_room_deducted = Receipt::where('ref_contract_id', $invoice_summarize->ref_contract_id)->where('payment_channel', 3)->where('ref_type_id', 1)->where('paid_on_checkout', 1)->latest()->first(); // ใบเสร็จค่าเช่าห้อง ที่ชำระโดย หักจากเงินประกัน
                if($receipt_rent_room_deducted){
                    foreach($receipt_rent_room_deducted->payment_list as $payment_list){
                            $pay_list = new PaymentList; // สร้างรายการ Invoice หักจากเงินประกัน ใน ใบย้ายออก
                            $pay_list->title  =  $payment_list->title;
                            $pay_list->price  =  $payment_list->price;
                            $pay_list->ref_payment_id  =  $request->invoice_id;
                ...
                $receipt_move_out_deducted = Receipt::where('ref_contract_id', $invoice_summarize->ref_contract_id)->where('payment_channel', 3)->where('ref_type_id', 4)->latest()->first(); // ใบเสร็จย้ายออก ที่ชำระโดย หักจากเงินประกัน
                if($receipt_move_out_deducted){
                    foreach($receipt_move_out_deducted->payment_list as $payment_list){
                            $pay_list = new PaymentList; // สร้างรายการ Invoice หักจากเงินประกัน ใน ใบย้ายออก
                            $pay_list->title  =  $payment_list->title;
                ...
                $receipt_move_out_find = Receipt::where('ref_contract_id', $invoice_summarize->ref_contract_id)->where('ref_type_id', 4)->latest()->first(); // ใบเสร็จย้ายออก ที่ชำระโดย หักจากเงินประกัน
                if($receipt_move_out_find){
                    foreach($receipt_move_out_find->payment_list as $payment_list){
                        if (strpos($payment_list->title, 'ค่าน้ำ') !== false) {
                            
                            $latestMeter = Meter::where('ref_room_id', $request->room_id)
                                                        ->orderByDesc('year')
                                                        ->orderByDesc('month')
                                                        ->first();

                            $latestMeter->water_unit = $payment_list->unit;
                            $latestMeter->save();
                        }
                    }
                }
            ...
            $expenses = new IncomeExpenses;
            $expenses->type  =  $receipt->total_amount > 0 ? 1 : 2;
            $expenses->label  =  "ใบย้ายออก";
            $expenses->amount  =  abs($receipt->total_amount);
            $expenses->date  =  Carbon::now();
            $expenses->time  =  date('H:i:s');
            $expenses->ref_room_id  =  $request->ref_room_id ?? $room->id;
            $expenses->ref_category_id  =  1;
    ...
    // ค่าน้ำ ค่าไฟฟ้า สุดท้าย
    public function get_last_meter($room_id)
    {
        try{
            $rent_bill = RentBill::where('ref_room_id', $room_id)->whereIn('ref_type_id', [1,4])->orderBy('year', 'DESC')->orderBy('month', 'DESC')->first();
            $meter = Meter::where('ref_room_id', $room_id)->orderBy('year', 'DESC')->orderBy('month', 'DESC')->first();
            return response()->json([
                'status'  => true,
                'old_water_unit' => floor($rent_bill->payment_water->unit),
                'old_electricity_unit' => floor($rent_bill->payment_electricity->unit),
                'water_unit' => floor($meter->water_unit),
                'electricity_unit' => floor($meter->electricity_unit)
            ]);
        } catch (QueryException $err) {
            DB::rollBack();
            return false;
        ...
        try{
            if($request->invoice_id){
                ...
                foreach($request->payment_list['title'] as $key => $title){
                    $unit = 0;
                    // $text = 'ค่าน้ำ (19 - 15 = 4 ยูนิต)';
                    if (strpos($title, 'ค่าน้ำ') !== false || strpos($title, 'ค่าไฟฟ้า') !== false) {
...
//   ชำระ ค่าจองหลายห้อง
    ...
    public function get_room_rental_contract($id)
    {
        ...
        $data['room_for_rent'] = RoomForRents::where(function ($q) {
                                                    $q->whereHas('rent_bills', function ($query) {
                                                            $query->where('ref_type_id', 3)
                                                                ->where('ref_status_id', 5);
                                                        })
    ...
    public function get_room_rental_move_out($id)
    {
        ...
        if(@$renter){
            ...
            $renter = [
                    "id" => "",
                    "ref_branch_id" => session("branch_id"),
                    "prefix" => "",
                    "name" => "",
                    "surname" => "",
                    "salary" => "",
                    ...
                    "id_card_number" => "",
                    "address" => null,
                    "ref_subdistrict_id" => null,
                    "ref_district_id" => null,
                    "ref_province_id" => null,
                    "zipcode" => null,
                    "birthdate" => "",
                    "booking_date" => "",
                    "booking_channel" => "",
                    "blacklist_status" => 0,
                    "blacklist_detail" => null,
                    "blacklist_date" => null,
                    "created_at" => "",
                    "updated_at" => "",
                    "fullName" => "",
    ...
    public function get_room_form_contract($id)
    {
        ...
        $contract = Renter::leftJoin('contracts', function ($join) use ($room) {
                                        $join->on('renters.id', '=', 'contracts.ref_renter_id')
                                            ->where('contracts.occupancy_id', $room->occupancy_id);
                                    })
                                ->leftJoin('room_for_rents', 'renters.id', '=', 'room_for_rents.ref_renter_id')
                                ->where('room_for_rents.occupancy_id', $room->occupancy_id)
                                ->select('contracts.*', 'renters.*', 'room_for_rents.deposit', 'contracts.id as contracts_id', 'contracts.address as contract_address', 'renters.address as renter_address', 'contracts.remark as contract_remark', 'room_for_rents.payment_received_date','contracts.id as contract_id','renters.id as renter_id', DB::raw("CONCAT(renters.name, ' ', IFNULL(renters.surname, '')) as full_name"))
                                ->orderBy('contracts.created_at', 'desc') // หรือใช้ 'id' ตามที่ต้องการ
                                ->first();
        ...
        $data['address'] = $contract->renter_address.' '.@$subdistrict->name_in_thai.' '.@$district.' '.@$province.' '.@$subdistrict->zip_code;

        if($contract->contracts_id != null){
            // return 123;
            $contract = Contract::leftJoin('renters','contracts.ref_renter_id' , '=', 'renters.id')
                                    ->leftJoin('room_for_rents', 'renters.id', '=', 'room_for_rents.ref_renter_id')
                                    ->where('contracts.occupancy_id', $room->occupancy_id)
            ...
            $data['address'] = $contract->contract_address;
        }

        // return 123;
        $data['receipt'] = Receipt::whereHas('invoice.room_for_rent', function ($query) use ($room) {
                                    $query->where('occupancy_id', $room->occupancy_id);
                                })->where('ref_room_id', $id)->where('ref_type_id', 3)->orderBy('id','DESC')->first();
    ...
    public function get_room_detail_contract($id)
    {
        ...
        $receipt_wait_for_confirm = Receipt::where('ref_room_id', $id)
                                                ...
                                                ->orderBy('id',"DESC")
                                                ->get(); // ใบเสร็จค่าประกัน รอคอนเฟิร์ม

        $receipt_deposit_and_jong = Receipt::where('ref_room_id', $id)
                                    ->whereIn('ref_type_id', [2,3])
                                    // ->where('ref_status_id', 5)
                                    ->whereHas('invoice.room_for_rent', function ($query) use ($room) {
                                        $query->where('occupancy_id', $room->occupancy_id);
                                    })
                                    ->orderBy('id',"DESC")
                                    ->get(); // ใบเสร็จค่าประกัน และ ค่าจอง
        ...
        $receipt = Receipt::where('ref_room_id', $id)
                                    ...
                                    ->where('ref_contract_id', $contract->contract_id)
                                    ->orderBy('id',"DESC")
                                    ->get(); // ใบเสร็จค่าประกัน

        $invoice_deposit_and_jong = RentBill::where('ref_room_id', $id)
                                    ->whereIn('ref_type_id', [2,3])
                                    // ->where('ref_status_id', 5)
                                    ->whereHas('room_for_rent', function ($query) use ($room) {
                                        $query->where('occupancy_id', $room->occupancy_id);
                                    })
                                    ->orderBy('id',"DESC")
                                    ->get(); // ใบแจ้งหนี้ค่าประกัน และ ค่าจอง

        $receipt_jong = Receipt::whereHas('invoice.room_for_rent', function ($query) use ($room) {
                                        $query->where('occupancy_id', $room->occupancy_id);
                                    })
        ...
        $data['receipt_jong'] = $receipt_jong;
        $data['receipt_deposit_and_jong'] = $receipt_deposit_and_jong;
        $data['invoice_deposit_and_jong'] = $invoice_deposit_and_jong;
    ...
    public function get_bill($id, $month)
    {
        $data['page_url'] = 'bill';
        $room = Room::find($id);
        // return $room->occupancy_id;
        $invoice = RentBill::where('ref_room_id', $id)
                            ->whereHas('room_for_rent', function ($query) use ($room) {
                                $query->where('occupancy_id', $room->occupancy_id);
                            })
                            ->whereNotIn('ref_status_id', [ 3, 5 ])
                            ->where('year', explode('-', $month)[0])
                            ->where('month', explode('-', $month)[1])  // พฤษภาคม
    ...
    public function change_room($old_room_id, $new_room_id)
    {
        try{
            ...
            $rfr = RoomForRents::where('ref_room_id', $old_room_id)->where('occupancy_id', $o_room->occupancy_id)->first();
            RoomForRents::where('ref_room_id', $old_room_id)->where('occupancy_id', $o_room->occupancy_id)->update([
                'ref_room_id' => $new_room_id
            ]);
            if($rfr){
                    
                Vehicle::where('ref_room_for_rent_id', $rfr->id)->update([
                    'ref_room_id' => $new_room_id
                ]);
                
                $contract = Contract::where('ref_room_id', $old_room_id)->where('occupancy_id', $o_room->occupancy_id)->first();
                
                Contract::where('ref_room_id', $old_room_id)->where('occupancy_id', $o_room->occupancy_id)->update([
                    'ref_room_id' => $new_room_id
                ]);
 
                RentBill::where('ref_room_id', $old_room_id)->where('ref_room_for_rent_id', $rfr->id)->update([
                    'ref_room_id' => $new_room_id
                ]);
            ...
            $n_room->occupancy_id  =  $o_room->occupancy_id;
            $n_room->status  =  $status;
            $n_room->save();

            $o_room->occupancy_id  =  0;
            $o_room->status  =  0;
            $o_room->save();
    ...
    public function insert_check_in(Request $request)
    {
        ...
        $occupancy_id = Room::orderBy('occupancy_id', 'DESC')->first()->occupancy_id + 1;
        // return $request;
        try{
            // return 123;
            ...
            if ($renter_blacklist) {
                return response()->json([
                    'status' => false,
                    'message' => 'บุคคลนี้ไม่สามารถ จอง ได้ เนื่องจากถูกขึ้นบัญชีดำ <br> เหตุผล '.$renter_blacklist->blacklist_detail.'<br>'.$renter_blacklist->room_for_rent->branch->name
                ]);
            }
            ...
            $renter->save();
                ...
                $r_f_r->occupancy_id  =  $occupancy_id;
                ...
                $r_f_r->save();
                    ...
                    $room->occupancy_id  =  $occupancy_id;
                    $room->status  =  1;
                    $room->save();
    ...
    // return $request;
        ...
        // return $request->contract;
            ...
            // return $row;
                ...
                $contract->occupancy_id  =  $room->occupancy_id;
                ...
                $r_b_room = new RentBill;  // สร้างบิลค่าเช่าห้อง สำหรับ 
...
$sequence++;
    ...
    $invoiceCode = 'INV'. $year
                ...
                $r_b->invoice_number  =  $invoiceCode;
                ...
                foreach($row['deposit'] as $r){
                    ...
                    $pay['payment_list']['id'][]  =  $pay_list->id;
                }
                if($request->check_in == null){
                    if($row['deduction_booking_amount'] > 0){
                        $pay_list = new PaymentList; // สร้างรายการ ค่าห้อง
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    public function insert_or_update_renter(Request $request)
    {
        ...
        $room = Room::find($request->room_id);
        // return $request;
        $room_for_rent = RoomForRents::find($request->room_for_rent_id);
        ...
        try{
            ...
            if(@$request->renter_id){
                ...
                Vehicle::where('ref_room_for_rent_id', $request->room_for_rent_id)->delete();
                ...
                $renter->surname  =  $request->surname;
                $renter->phone  =  $request->phone;
                $renter->salary  =  $request->salary ?? 0;
                $renter->id_card_number  =  $request->id_card_number;
                $renter->address  =  $request->address;
                $renter->ref_subdistrict_id  =  $request->ref_subdistrict_id;
                $renter->ref_district_id  =  $request->ref_district_id;
            ...
            if(!$request->renter_id){
                ...
                $r_t_r->occupancy_id  =  $room->occupancy_id;
        ...
        try{
            ...
            if($request->select_channel == 1){
                ...
                if(@$request->room_text){
                    ...
                    $room_all = Room::whereIn('id', $request->room_text_id)->where('status', 0)->get();
                ...
                foreach($room_all as $r_n){
                    ...
                    $r_f_r->ref_renter_id  =  $renter->id;
                    $r_f_r->ref_user_id  =  Auth::id();
                    $r_f_r->deposit  =  $request->deposit ?? 0;
                    $r_f_r->payment_method  =  $request->payment_channel ?? 1;
                    $r_f_r->payment_received_date  =  $payment_received_date;
                    $r_f_r->save();
                    
                    $update_room = Room::find($r_n->id);
                    $update_room->occupancy_id  =  $occupancy_id;
                    ...
                    if($request->deposit_status == 1){
                        ...
                        $booking = new Booking;
                        $booking->occupancy_id  =  $occupancy_id;
                        $booking->ref_room_id  =  $r_n->id;
                        $booking->ref_room_for_rent_id  =  $r_f_r->id;
                        $booking->save();
                }
            }else{
