            ...
            @if ($list_data->lastPage() > 1)
                <ul class="pagination">
                    ...
                    <li class="page-item {{ ($list_data->currentPage() == 1) ? ' disabled' : '' }}">
                        <a class="page-link" href="javascript:void(0)" onclick='loadInvoiceData("{{ $list_data->url(1) }}")'>First</a>
                    </li>
                    ...
                    @for ($i = $from; $i <= $to; $i++)
                        <li class="page-item {{ ($list_data->currentPage() == $i) ? ' active' : '' }}">
                            <a class="page-link" href="javascript:void(0)" onclick='loadInvoiceData("{{ $list_data->url($i) }}")'>{{ $i }}</a>
                        </li>
                    @endfor
                    ...
                    @if ($to < $list_data->lastPage())
                        ...
                        <li class="page-item">
                            <a class="page-link" href="javascript:void(0)" onclick='loadInvoiceData("{{ $list_data->url($list_data->lastPage()) }}")'>{{ $list_data->lastPage() }}</a>
                        </li>
                    @endif

                    <!-- ปุ่ม Last -->
                    <li class="page-item {{ ($list_data->currentPage() == $list_data->lastPage()) ? ' disabled' : '' }}">
                        <a class="page-link" href="javascript:void(0)" onclick='loadInvoiceData("{{ $list_data->url($list_data->lastPage()) }}")'>Last</a>
                    </li>
                </ul>
            @endif
        </div>
