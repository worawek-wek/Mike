   <table class="datatables-basic table dataTable no-footer dtr-column"
        ...
        <tbody>
            ...
            <tr class="odd">
                ...
                <td class="text-left">{{ $row->renter->room_for_rent->room->name ?? '-' }}</td>
                <td class="text-center">{{ $row->renter->prefix ?? '' }} {{ $row->renter->name ?? '' }} {{ $row->renter->surname ?? '' }}</td>
            ...
            @if ($list_data->lastPage() > 1)
                <ul class="pagination">
                    ...
                    <li class="page-item {{ ($list_data->currentPage() == 1) ? ' disabled' : '' }}">
                        <a class="page-link" href="javascript:void(0)" onclick='loadOldData("{{ $list_data->url(1) }}")'>First</a>
                    </li>
                    ...
                    @for ($i = $from; $i <= $to; $i++)
                        <li class="page-item {{ ($list_data->currentPage() == $i) ? ' active' : '' }}">
                            <a class="page-link" href="javascript:void(0)" onclick='loadOldData("{{ $list_data->url($i) }}")'>{{ $i }}</a>
                        </li>
                    @endfor
                    ...
                    @if ($to < $list_data->lastPage())
                        ...
                        <li class="page-item">
                            <a class="page-link" href="javascript:void(0)" onclick='loadOldData("{{ $list_data->url($list_data->lastPage()) }}")'>{{ $list_data->lastPage() }}</a>
                        </li>
                    @endif

                    <!-- ปุ่ม Last -->
                    <li class="page-item {{ ($list_data->currentPage() == $list_data->lastPage()) ? ' disabled' : '' }}">
                        <a class="page-link" href="javascript:void(0)" onclick='loadOldData("{{ $list_data->url($list_data->lastPage()) }}")'>Last</a>
                    </li>
                </ul>
            @endif
        </div>
