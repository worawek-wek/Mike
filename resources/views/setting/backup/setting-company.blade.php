                                            ...
                                            <div class="col-sm-6 text-end">
                                                <button type="button" class="btn btn-main waves-effect waves-light"
                                                    data-bs-toggle="modal" data-bs-target="#addModal"><i
                                                        class="ti-xs ti ti-plus me-2"></i>เพิ่มบริษัท</button>
                                            </div>
                                        </div>
                                    </div>
            ...
            <form id="insert_company">
                ...
                <div class="modal-content rounded-0">
                    <div class="modal-header rounded-0">
                        <h5 class="modal-title" id="exampleModalLabel1">เพิ่มข้อมูลบริษัท</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    @include('setting/setting-company-form')
                </div>
            </form>
        </div>
    </div>
    ...
    <div class="modal fade modalHeadDecor" id="editModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg" role="document">
            <form id="update_company">
                ...
                <div class="modal-content rounded-0">
                    <div class="modal-header rounded-0">
                        <h5 class="modal-title" id="exampleModalLabel1">แก้ไขข้อมูลบริษัท</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div id="view">
...
<script>
    var page = "{{$page_url}}/datatable";
        ...
        $('#insert_company').on('submit', function(event) {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response == true){
                                ...
                                Swal.fire('เพิ่มบริษัทเรียบร้อยแล้ว', '', 'success');
                                $('#addModal').modal('hide');
                                loadData(page);
                            }
                        },
        ...
        $('#update_company').on('submit', function(event) {
            ...
            Swal.fire({
                title: 'ยืนยันการดำเนินการ?',
                text: 'คุณต้องการแก้ไขบริษัทหรือไม่?',
                icon: 'warning',
                showCancelButton: true,
                confirmButtonText: 'ตกลง',
                cancelButtonText: 'ยกเลิก',
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        ...
                        success: function(response) {
                            if(response == true){
                                Swal.fire('แก้ไขบริษัทเรียบร้อยแล้ว', '', 'success');
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(response) {
                                if(response == true){
                                    loadData(page);
                                    Swal.fire('ลบบริษัทเรียบร้อยแล้ว', '', 'success');
